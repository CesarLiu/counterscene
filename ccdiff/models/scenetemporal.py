...
class SceneTransformer(nn.Module):
    ...
    def __init__(self, d_k=128, d_edge=32, L_enc=1, dropout=0.0,
                 ...
                 use_conflict_soft_gate=False, conflict_gate_hidden_dim=64, conflict_gate_eps=1e-4,
        ...
        if self.use_conflict_soft_gate:
            self.conflict_gate = nn.Sequential(
                init_(nn.Linear(self.d_edge, conflict_gate_hidden_dim)),
                nn.Mish(),
                init_(nn.Linear(conflict_gate_hidden_dim, 1)),
            )
    ...
    def social_attn_with_edge_decoder_fn(self, agents_emb, agent_masks, layer, aux_info, attn_name="", T_total=83, motion_dist='ttc', use_graph_cond=False):
        ...
        if self.mask_social_interaction:
            ...
            if self.interaction_edge_speed_repr in ['rel_vel_per_step']:
                ...
                if motion_dist == 'dist':
                    ...
                    neighbor_dist = aux_info['neighbor_feat'][..., 10].permute(0,3,1,2) # TTC values
                elif motion_dist == 'cig_soft':
                    neighbor_dist = None
                else:
                    raise NotImplementedError("{} not implemented".format(motion_dist))

            if motion_dist == 'cig_soft':
                neighbor_dist_mask = torch.zeros_like(
                    neighbor_mask.permute(0, 3, 1, 2), dtype=torch.bool
                )
            elif not use_graph_cond:
        ...
        gate_bias = None
        if self.use_conflict_soft_gate and motion_dist == 'cig_soft' and not use_graph_cond:
            edge_gate = torch.sigmoid(self.conflict_gate(edge_feat).squeeze(-1))
            gate_bias = torch.log(edge_gate.clamp_min(self.conflict_gate_eps))
            gate_bias = gate_bias.permute(0, 3, 1, 2).unsqueeze(-2)
            gate_bias = gate_bias.repeat(1, 1, 1, M, 1).view(B*T_all, M, M*M)
            gate_bias = gate_bias.unsqueeze(1).repeat(1, self.num_heads, 1, 1)
            gate_bias = gate_bias.view(B*T_all*self.num_heads, M, M*M)
        ...
        if gate_bias is not None:
            attn_mask = gate_bias.masked_fill(attn_mask, -1e4)
