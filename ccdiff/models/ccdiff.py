...
from tbsim.utils.guidance_loss import verify_guidance_config_list, verify_constraint_config, apply_constraints
from ccdiff.utils.guidance_loss import PartialPerturbationGuidance
...
class CCDiffuserModel(nn.Module):
    ...
    def __init__(
            ...
            use_conflict_soft_gate: bool = False,
            conflict_gate_hidden_dim: int = 64,
            conflict_gate_eps: float = 1e-4,
            cig_horizon_steps: int = 52,
            cig_ttc_cap: float = 5.2,
            cig_distance_cap: float = 20.0,
            cig_speed_cap: float = 20.0,
        ...
        self.use_conflict_soft_gate = use_conflict_soft_gate
        self.conflict_gate_hidden_dim = conflict_gate_hidden_dim
        self.conflict_gate_eps = conflict_gate_eps
        self.cig_horizon_steps = cig_horizon_steps
        self.cig_ttc_cap = cig_ttc_cap
        self.cig_distance_cap = cig_distance_cap
        self.cig_speed_cap = cig_speed_cap
        ...
        if self.diffuser_model_arch == "SceneTransformer":
            ...
            self.model = SceneTransformer(
                                    ...
                                    use_conflict_soft_gate=self.use_conflict_soft_gate,
                                    conflict_gate_hidden_dim=self.conflict_gate_hidden_dim,
                                    conflict_gate_eps=self.conflict_gate_eps,
    ...
    def prepare_scene_agent_hist(self, pos, yaw, speed, extent, avail, norm_info, scale=True, speed_repr='abs_speed'):
        ...
        if speed_repr in ['rel_vel', 'rel_vel_per_step', 'rel_vel_new', 'rel_vel_new_new']:
            ...
            raw_ttc = rel_t_to_col.unsqueeze(-1)
            rel_t_to_col = (t_to_col_th - raw_ttc) / t_to_col_th
            ...
            cig_extra = None
            if self.motion_dist == 'cig_soft':
                if speed_repr == 'rel_vel_new_new':
                    # Preserve the feature convention used to train the paper
                    # checkpoint: axis-wise relative displacement magnitudes.
                    relative_position = pos_diff
                else:
                    relative_position = pos
                cig_extra, cig_info = compute_cig_features(
                    ...
                    raw_ttc=raw_ttc,
                    dt=self.dt,
                    horizon_steps=self.cig_horizon_steps,
                    distance_cap=self.cig_distance_cap,
                    speed_cap=self.cig_speed_cap,
                    ttc_cap=self.cig_ttc_cap,
                )
                info.update(cig_info)
        ...
        if speed_repr in ['rel_vel', 'rel_vel_new', 'rel_vel_per_step', 'rel_vel_new_new']:
            ...
            if speed_repr in ['rel_vel_per_step', 'rel_vel_new', 'rel_vel_new_new']:
                ...
                edge_parts = [pos, hvec, vvec, lw, rel_d, rel_d_lw, rel_t_to_col]
                if cig_extra is not None:
                    edge_parts.append(cig_extra)
                edge_parts.extend([pos_original, avail.unsqueeze(-1)])
                hist_in = torch.cat(edge_parts, dim=-1)
                ...
                if cig_extra is not None:
                    edge_parts.append(cig_extra)
                edge_parts.append(avail.unsqueeze(-1))
                hist_in = torch.cat(edge_parts, dim=-1)
