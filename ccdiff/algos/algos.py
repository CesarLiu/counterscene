...
class CCDiffTrafficModel(pl.LightningModule):
    def __init__(self, algo_config, modality_shapes, registered_name, do_log=True, guidance_config=None, constraint_config=None):
        ...
        self.nets["policy"] = CCDiffuserModel(
            ...
            motion_dist=getattr(algo_config, 'motion_dist', 'ttc'),
            social_attn_radius=algo_config.social_attn_radius,
            use_conflict_soft_gate=getattr(algo_config, 'use_conflict_soft_gate', False),
            conflict_gate_hidden_dim=getattr(algo_config, 'conflict_gate_hidden_dim', 64),
            conflict_gate_eps=getattr(algo_config, 'conflict_gate_eps', 1e-4),
            cig_horizon_steps=getattr(algo_config, 'cig_horizon_steps', algo_config.horizon),
            cig_ttc_cap=getattr(algo_config, 'cig_ttc_cap', algo_config.horizon * algo_config.step_time),
            cig_distance_cap=getattr(algo_config, 'cig_distance_cap', 20.0),
            cig_speed_cap=getattr(algo_config, 'cig_speed_cap', 20.0),
