...
def parse_single_result(
    ...
    if estimate_dist:
        ...
        hjson = compute_and_save_hist(
            os.path.join(results_dir, "data.hdf5"),
            eval_out_dir,
            save_hist_data,
            normalization_by=normalization_by,
            disable_control_on_stationary=disable_control_on_stationary,
        )

        if gt_hist_path is None:
            raise ValueError("--estimate_dist requires an explicit --gt_hist path")
...
def parse(args):
    ...
    if args.results_set is not None:
        ...
        raise ValueError("provide either --results_dir or --results_set")
    print(all_results)
    ...
    for result_path in all_results:
        if os.path.isdir(result_path):
            ...
            agg_res = parse_single_result(
                result_path,
                eval_out_dir,
                args.save_hist_data,
                gt_hist_path=args.gt_hist,
                estimate_dist=args.estimate_dist,
                normalization_by=args.normalization_by,
                disable_control_on_stationary=args.disable_control_on_stationary,
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="A rollout directory containing stats.json and optionally data.hdf5"
    )
