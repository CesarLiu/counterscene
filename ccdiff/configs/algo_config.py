...
class CounterSceneConfig(CCDiffConfig):
    """Paper configuration with the conflict interaction graph enabled."""

    def __init__(self):
        super(CounterSceneConfig, self).__init__()
        self.name = "counterscene"
        # Edge layout in CIG mode:
        # Δp(0:2), heading(2:4), Δv(4:6), extent(6:8), distance(8),
        # box distance(9), TTC(10), TTI(11), d_int(12), v_int(13), ...
        self.neighbor_inds = [0, 1, 4, 5, 10, 11, 12, 13]
        self.edge_attr_separation = [[0, 1], [2, 3], [4, 5, 6, 7]]
        self.motion_dist = "cig_soft"
        self.use_conflict_soft_gate = True
        self.conflict_gate_hidden_dim = 64
        self.conflict_gate_eps = 1e-4
        self.cig_horizon_steps = self.horizon
        self.cig_ttc_cap = self.horizon * self.step_time
        self.cig_distance_cap = 20.0
        self.cig_speed_cap = 20.0
