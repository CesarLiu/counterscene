...
class GuidanceMetric(EnvMetrics):
    def __init__(self, scene_idx, params, agents, num_scenes):
        ...
        self.params = params
        self.agents = None if agents == 'all' else agents
...
def guidance_metrics_from_config(guidance_config):
    ...
    for si in range(num_scenes):
        ...
        for ci, cur_cfg in enumerate(cur_cfg_list):
            ...
            elif cur_cfg['name'] == 'conflict_point_guidance_v3':
                # The objective has no separate rollout metric. Core collision
                # and off-road metrics are still reported by the environment.
                continue
            else:
