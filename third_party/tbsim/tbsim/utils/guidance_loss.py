...
def choose_action_from_guidance(preds, obs_dict, guide_configs, guide_losses):
    ...
    for sidx in range(len(guide_configs)):
        ...
        is_scene_level = np.array([guide_cfg.name in ['agent_collision', 'social_group', 'gptcollision', 'gptkeepdistance', 'conflict_point_guidance_v3'] for guide_cfg in scene_guide_cfg])
...
GUIDANCE_FUNC_MAP = {
    ...
    'conflict_point_guidance_v3' : ConflictPointGuidanceLossV3,
}
        ...
        for si in range(self.num_scenes):
            ...
            if len(cur_guide) > 0:
                ...
                for gidx, guide_cfg in enumerate(cur_guide):
                    ...
                    if guide_cfg.agents is not None and guide_cfg.agents != 'all':
