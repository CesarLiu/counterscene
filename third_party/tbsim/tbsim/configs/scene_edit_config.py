...
class SceneEditingConfig(EvaluationConfig):
    def __init__(self, registered_name='trajdata_nusc_counterscene'):
        ...
        self.policy.num_action_samples = 5
        ...
        if 'nusc' in registered_name:
            ...
            if 'ngc' in registered_name:
                self.trajdata.trajdata_cache_location = os.environ.get(
                    "TRAJDATA_CACHE_DIR", "/workspace/unified_data_cache"
                )
                self.trajdata.trajdata_data_dirs = {
                "nusc_trainval" : os.environ.get("NUSCENES_ROOT", "/workspace/nuscenes/"),
                }
            else:
                self.trajdata.trajdata_cache_location = os.environ.get(
                ...
                self.trajdata.trajdata_data_dirs = {
                    "nusc_trainval" : os.environ.get("NUSCENES_ROOT", "data/nuscenes"),
                }
            ...
            raise ValueError(f"unsupported registered_name: {registered_name}")
